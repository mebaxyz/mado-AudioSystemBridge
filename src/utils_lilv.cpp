...
// Delegating wrapper — migrated to LV2PluginScanner
const PluginInfo_Mini* _get_plugin_info_mini(LilvWorld* const w,
                                             const LilvPlugin* const p,
                                             const NamespaceDefinitions& ns)
{
    // Use a temporary scanner that will not take ownership of the world pointer
    LV2PluginScanner scanner(w);
    return scanner.getPluginInfoMini(w, p, ns);
}
...
void _refresh()
{
    ...
    // Delegate cache population to the scanner singleton. This migrates
    // the legacy per-plugin loop into LV2PluginScanner::populateCaches().
    const LilvPlugins* plugins = lilv_world_get_all_plugins(w);
    LV2PluginScanner::global().setPlugins(plugins);
    LV2PluginScanner::global().populateCaches(w);
}

// Small C wrappers to allow incremental migration of global plugin caches into LV2PluginScanner
extern "C" bool plugin_should_reload(const char* uri)
{
    return LV2PluginScanner::global().pluginShouldReload(uri);
}

extern "C" void plugin_remove_from_reload(const char* uri)
{
    LV2PluginScanner::global().pluginRemoveFromReload(uri);
}

extern "C" const LilvPlugins* get_lilv_plugins()
{
    return LV2PluginScanner::global().getPlugins();
}
...
{
    LV2PluginScanner scanner(w);
    scanner.placePresetInfo(w, info, p, pset_Preset, rdfs_label);
}
...
{
    LV2PluginScanner scanner(nullptr);
    return scanner.getPluginCategories(p, rdf_type, supported);
}
    ...
    if (LilvNodes* const blocks = lilv_plugin_get_value(p, ingenblocknode))
    {
        ...
        LILV_FOREACH(nodes, itblocks, blocks)
        {
            ...
            if (LilvNode* const proto = lilv_world_get(w, block, lv2protonode, nullptr))
            {
                ...
                {
                        if (LV2PluginScanner::global().getPluginMini(uri) == nullptr)
                        {
                            LilvNode* const urinode = lilv_new_uri(w2, uri.c_str());
                            const LilvPlugins* plugins_for_lookup = LV2PluginScanner::global().getPlugins();
                            const PluginInfo_Mini* mini = _get_plugin_info_mini(w2, lilv_plugins_get_by_uri(plugins_for_lookup, urinode), ns);
                            LV2PluginScanner::global().setPluginMini(uri, mini);
                            lilv_node_free(urinode);
                        }

                        if (const PluginInfo_Mini* const miniInfo = LV2PluginScanner::global().getPluginMini(uri))
    ...
    if (LilvNodes* const blocks = lilv_plugin_get_value(p, ingenblocknode))
    {
        LILV_FOREACH(nodes, itblocks, blocks)
        {
            ...
            if (LilvNode* const proto = lilv_world_get(w, block, lv2protonode, nullptr))
            {
                ...
                if (! LV2PluginScanner::global().hasPluginInfo(uri))
                {
                    broken = true;
                    break;
                }
...
{
    LV2PluginScanner scanner(w);
    return scanner.fillPluginInfoWithPresets(w, info, uri);
}
...
void cleanup(void)
{
    ...
    // Delegate cache clearing to the scanner singleton (owns plugin/bundle caches)
    LV2PluginScanner::global().clearCaches();
...
bool is_bundle_loaded(const char* const bundle)
{
    ...
    // ask scanner whether this bundle was recorded as loaded
    return LV2PluginScanner::global().isBundleLoaded(bundlepath);
}
...
const char* const* add_bundle_to_lilv_world(const char* const bundle)
{
    ...
    if (LV2PluginScanner::global().isBundleLoaded(bundlepath))
        return nullptr;
    ...
    // refresh the scanner's plugin pointer and populate caches
    const LilvPlugins* all = lilv_world_get_all_plugins(w);
    LV2PluginScanner::global().setPlugins(all);
    LV2PluginScanner::global().populateCaches(w);

    // add to loaded list via scanner (scanner tracks bundles_ internally)
    LV2PluginScanner::global().addBundle(bundlepath);
    ...
    if (LilvWorld* const w2 = lilv_world_new())
    {
        ...
        LILV_FOREACH(plugins, itpls, plugins)
        {
            ...
            // store new empty data (migrated to scanner)
            LV2PluginScanner::global().setPluginInfo(uri, PluginInfo_Init);
            LV2PluginScanner::global().setPluginMini(uri, nullptr);
...
const char* const* remove_bundle_from_lilv_world(const char* const bundle, const char* const resource)
{
    ...
    // stop now if bundle is not loaded (ask scanner)
    if (! LV2PluginScanner::global().isBundleLoaded(bundlepath))
        return nullptr;
    ...
    // remove from loaded list via scanner
    LV2PluginScanner::global().removeBundle(bundlepath, resource ? std::string(resource) : std::string());

    std::vector<const char*> removedPlugins;

    // remove all plugins that are present on that bundle
    {
        const LilvPlugins* plugins = LV2PluginScanner::global().getPlugins();
        if (plugins)
        {
            LILV_FOREACH(plugins, itpls, plugins)
            {
        ...
        LILV_FOREACH(nodes, itbnds, bundles)
        {
            ...
            if (bundlepath != bundleparsed)
                ...
                // clear via scanner
                LV2PluginScanner::global().clearPluginInfo(uri);
    ...
    // refresh the scanner's plugin pointer and populate caches
    const LilvPlugins* all2 = lilv_world_get_all_plugins(w);
    LV2PluginScanner::global().setPlugins(all2);
    LV2PluginScanner::global().populateCaches(w);
...
{
    const LilvPlugins* plugins = LV2PluginScanner::global().getPlugins();
    const int newsize = (int)(plugins ? lilv_plugins_size(plugins) : 0);
...
const PluginInfo_Mini* const* get_all_plugins(void)
{
    ...
    if (plugins)
    {
        ...
        // check if it's already cached in the scanner
        if (LV2PluginScanner::global().getPluginMini(uri) == nullptr)
            LV2PluginScanner::global().setPluginMini(uri, _get_plugin_info_mini(w, p, ns));

        if (const PluginInfo_Mini* const miniInfo = LV2PluginScanner::global().getPluginMini(uri))
        {
            if (kOnlyShowPluginsWithMODGUI && miniInfo->gui.resourcesDirectory == nc)
                continue;
            _get_plugs_mini_ret[curIndex++] = miniInfo;
        }
    }
...
const PluginInfo* get_plugin_info(const char* const uri_)
{
    ...
    if (PluginInfo* pCached = LV2PluginScanner::global().getPluginInfoPtr(uri))
        if (pCached->valid)
            return _fill_plugin_info_with_presets(w, *pCached, uri);
    ...
    const LilvPlugins* plugins = LV2PluginScanner::global().getPlugins();
    const LilvPlugin* const p = (plugins ? lilv_plugins_get_by_uri(plugins, urinode) : nullptr);
    lilv_node_free(urinode);
    ...
    if (p != nullptr)
    {
        ...
        LV2PluginScanner::global().setPluginInfo(uri, pInfo);

        // fill mini-info into a temporary pointer and store it in the scanner
        PluginInfo_Mini* mini_tmp = nullptr;
        _fill_plugin_info_mini_from_full(pInfo, &mini_tmp);
        LV2PluginScanner::global().setPluginMini(uri, mini_tmp);

        return LV2PluginScanner::global().getPluginInfoPtr(uri);
    }
...
const PluginGUI* get_plugin_gui(const char* uri_)
{
    ...
    // check if it exists via scanner
    if (! LV2PluginScanner::global().hasPluginInfo(uri))
        return nullptr;
    ...
    const LilvPlugins* plugins_for_lookup = LV2PluginScanner::global().getPlugins();
    const LilvPlugin* const p = lilv_plugins_get_by_uri(plugins_for_lookup, urinode);
    lilv_node_free(urinode);
    ...
    if (p != nullptr)
    {
        ...
        PluginInfo* pNew = LV2PluginScanner::global().getPluginInfoPtr(uri);
        if (pNew) return &pNew->gui;
    }
...
const PluginGUI_Mini* get_plugin_gui_mini(const char* uri_)
{
    ...
    // look for it using the scanner's plugin list
    {
        const LilvPlugins* plugins = LV2PluginScanner::global().getPlugins();
        ...
        LILV_FOREACH(plugins, itpls, plugins)
        {
            ...
            {
                LV2PluginScanner::global().setPluginMini(uri, miniInfo);
                return &miniInfo->gui;
            }
...
const PluginPort* get_plugin_control_inputs(const char* const uri_)
{
    ...
    // check if plugin is present in scanner cache
    if (PluginInfo* pCached = LV2PluginScanner::global().getPluginInfoPtr(uri))
    {
        if (pCached->valid)
            return pCached->ports.control.input;
    }

    LilvWorld* const w = W;

    // look for it in the world via the scanner's plugin list
    {
        const LilvPlugins* plugins = LV2PluginScanner::global().getPlugins();
        if (! plugins) return nullptr;
        ...
        LILV_FOREACH(plugins, itpls, plugins)
        {
            ...
            PluginInfo* pNew = LV2PluginScanner::global().getPluginInfoPtr(uri);
            if (pNew) return pNew->ports.control.input;
        }
    }
...
const PluginInfo_Essentials* get_plugin_info_essentials(const char* const uri_)
{
    ...
    // check if plugin is already cached in scanner
    if (PluginInfo* pCached = LV2PluginScanner::global().getPluginInfoPtr(uri))
    {
        if (pCached->valid)
        {
            info.controlInputs    = pCached->ports.control.input;
            info.monitoredOutputs = pCached->gui.monitoredOutputs;
            info.parameters       = pCached->parameters;
            info.buildEnvironment = pCached->buildEnvironment;
            info.microVersion     = pCached->microVersion;
            info.minorVersion     = pCached->minorVersion;
            info.release          = pCached->release;
            info.builder          = pCached->builder;
            return &info;
        }
    }
...
bool is_plugin_preset_valid(const char* const plugin_, const char* const preset)
{
    ...
    // plugin must be cached in the scanner
    PluginInfo* pCached = LV2PluginScanner::global().getPluginInfoPtr(plugin);
    if (pCached == nullptr || ! pCached->valid)
        return false;

    const PluginInfo& pInfo = *pCached;
...
void rescan_plugin_presets(const char* const uri_)
{
    ...
    LV2PluginScanner::global().pluginAddToReload(uri);
}
